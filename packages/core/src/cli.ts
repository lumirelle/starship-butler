...
cli
  .command('set <source-pattern> <target>', 'Let butler setting up locally, notice that the source glob pattern is relative to the assets folder.')
  .option('-f, --force', 'Configure locally and forcibly, override the existing configuration with the same name')
  .option('-?, --verbose', 'Show verbose output')
  .option('-d, --dry-run', 'Dry run')
  .option('-s, --symlink', 'Symlink configuration instead of copy and paste')
  .action(async (sourcePattern: string, target: string, options: Partial<SetOptionsFromCommandLine>) => {
    // Use type to limit the usage of config options
    ...
    const mergedOptions = mergeOptions(configOptions, options)
    await settingUp(sourcePattern, target, mergedOptions)
  })
