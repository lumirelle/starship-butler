...
cli
  ...
  .alias('cfsys')
  .option('-i, --includeOnly [actionIds]', 'Preset configuring actions that are included only. Notice that, `includeOnly` has higher priority than `exclude`.')
  .option('-x, --exclude [actionIds]', 'Preset configuring actions that are excluded. Notice that, `includeOnly` has higher priority than `exclude`.')
...
cli
  .command('set <source-pattern> <target>', 'Let butler setting up locally, `source-pattern` is support both file path and glob pattern.')
