...
export const windowsTerminal: ActionFactory = () => {
  ...
  return {
    ...
    posthandler: ({ destination }) => {
      consola.info(
        `This configuration will use \`'"Comic Mono", "Maple Mono CN", "Symbols Nerd Font Mono"'\` as terminal fonts, \`Nushell\` as default shell. If you don't want to use them, please edit this config \`(${join(destination, 'settings.json')})\` manually or consider not to include this action.`,
      )
    },
  }
}
